                            ...
                            <table class="table table-striped table-bordered table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th>购买 {{trans('home.invoice_table_create_date')}} </th>
                                        <th>到期时间</th>
                                ...
                                @else
                                    @foreach($orderList as $key => $order)
                                        <tr class="odd gradeX">
                                           ...
                                           <td>{{date('Y-m-d H:i:s', strtotime($order->expire_at))}}</td>
                                            <td>
                                                @if(!$order->is_expire)
                                                    @if($order->status == 0)
                                                    ...
                                                    @elseif($order->status == 2)
                                                        ...
                                                        <span class="label label-success">续费{{trans('home.invoice_table_wait_active')}} </span>
                                                         @else 
