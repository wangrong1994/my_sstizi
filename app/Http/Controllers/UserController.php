...
class UserController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    /*$order = Order::query()->where('user_id', $user->id)->where('is_expire', 0)->where('status', 2)->orderBy('oid', 'desc')->first();
     if (!empty($order)) {
       ...
       $user->expire_time = $order->expire_at;
     }*/
   
    /*$user->is_reg = 0;
    $is_buy = Order::query()->where('user_id', $user->id)->orderBy('oid', 'desc')->first();
    if (empty($is_buy)) {
       $user->is_reg = 1;
    } 
   */
    $user->laster_use =  $user->t;
    $user->now = date('Y-m-d H:i:s'); 
    
    $view['info'] = $user->toArray();
    $view['notice'] = Article::query()->where('type', 2)->where('is_del', 0)->orderBy('id', 'desc')->paginate(6)->appends($request->except('page'));
  ...
  public function account(Request $request)
  {
    ...
    $nodeList = DB::table('ss_group_node')
      ...
      ->where('ss_group.level', '<=', $user->level) //可显示小于用户等级的节点信息
      ->where('ss_node.status', 1)
      ->orderBy('ss_node.sort', 'desc')
      ->get();

    foreach ($nodeList as &$node) {
       //if ($node->group_id==5 && $user->level>1) {
         //  continue;
       //}
      // 生成ssr scheme
      ...
      // 生成ss scheme
      $remark = $this->base64url_encode($node->name);
      $userinfo = $user->method.':'.$user->passwd."@".$node->server.":".$user->port;
      $ss_scheme = 'ss://' . $this->base64url_encode($userinfo);
      ...
      // 生成文本配置信息
      $node->port = $node->single ? $node->single_port : $user->port;
      $node->passwd = $node->single ? $node->single_passwd : $user->passwd;
      $node->method = $node->single ? $node->single_method : $user->method;
      $node->local_port = 1080;
      $node->ssr_scheme = $ssr_scheme;
      $node->ss_scheme = $node->compatible ? $ss_scheme : ''; // 节点兼容原版才显示
      $country= Country::query()->where('country_code', $node->country_code)->first();
      $node->country_name = $country->country_name;
     // $node_data = $node->toArray();
    }
    $view['info'] = $user->toArray();
    $view['nodeList'] = $nodeList;
    return Response::view('user/account', $view);
  }
  ...
  public function goodsList(Request $request)
  {
      ...
      foreach ($goodsList as $goods) {
          $goods->year_price = $goods->year_price / 100;
          $goods->mon_price = $goods->mon_price / 100;
      ...
      if (!empty($order)) {
        //获取有效订单的商品等级
        $order_goods = Goods::query()->where('id', $order->goods_id)->first();//前端判断id是否相等，不相等则表示不能购买
        $view['specify'] = 1;
        $view['order_goods']= $order_goods['id'];
      }
  ...
  public function reset(Request $request, $token)
  {
      if ($request->method() == 'POST') {
          ...
          // 更新密码
          $ret = User::query()->where('id', $verify->user_id)->update(['password' => md5($password.$verify->user->username)]);
...
public function createOrder (Request $request) {
  ...
  if ($request->method() == 'POST') {
    ...
    //新增支付
     $view['alipay_status'] = self::$config['alipay_status'];
     $view['stripe_status'] = self::$config['stripe_status'];
          ...
          try {
             ...
             /* if ($goods->type == 2) {
                  ...
                  foreach ($existOrderList as $vo) {
                      Order::query()->where('oid', $vo->oid)->update(['is_expire' => 1, 'transfer_enable' => 0]);
                  }
              }*/
              ...
              if ($goods->type == 2) {
                  ...
                  User::query()->where('id', $user->id)->update(['traffic_reset_day' => $traffic_reset_day, 'expire_time' => date('Y-m-d', strtotime("+" . $goods->days . " days", time())), 'enable' => 1]);
              } else {
                  // 将商品的有效期和流量自动重置日期加到账号上
  ...
  //用户账号修改密码
  public function modify_passwd (Request $request) {
    $user = $request->session()->get('user');
    $user = User::query()->where('id', $user['id'])->first();
    $new_passwd = trim(strval($request->get('new_passwd')));
    if ($request->method() == 'POST') {
      if (empty($new_passwd)) {
        return Response::json(['status' => 'fail', 'data' => '', 'message' => '请填写新密码！']);
      }

      $user = User::query()->where('id', $user->id)->update(['passwd' => $new_passwd]);
      if (!$user) {
        return Response::json(['status' => 'fail', 'data' => '', 'message' => '修改密码失败，请稍后再试。']);
      }
      return Response::json(['status' => 'success', 'data' => '', 'message' => '修改成功！']);
    }
    else {
      $view['user'] = $user->toArray();
      return Response::view('user/modify_passwd', $view);
    }
  }
