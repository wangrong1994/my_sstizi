...
public function test(){
     $obj= DB::select('select b.id from user a,user b where a.id=b.id-1 and b.port-a.port>1 limit 1');
...
foreach ($obj as $user) {
     ...
     $last_user = DB::select('select a.id,a.port from (select @rowno:=@rowno+1 rowno,r.* from user r,(select @rowno:=0) t order by r.port) a,(select @rowno_1:=@rowno_1+1 rowno,r.* from user r,(select @rowno_1:=0) t order by r.port) b where a.rowno=b.rowno-1 and b.port-a.port>1 and a.port !=0 and a.enable=1 limit 1
      ...
      if ($last_user) {
                  foreach ($last_user as $port_user)  {
                    $view['last_port'] = $port_user->port + 1;
                   // Log::info('新的端口：'.$port);
                  }
       }               
      else {
          $last_user2 = User::query()->orderBy('id', 'desc')->first();
          $view['last_port'] = self::$config['is_rand_port'] ? $this->getRandPort() : $last_user2->port + 1;
       }               
  ...
  public function addNode(Request $request)
  {
    if ($request->method() == 'POST') {
      ...
      $group_id = $request->get('group_id'); //所属级别
      ...
      if (empty($name) || empty($group_id) || empty($country_code) || empty($server) || empty($method)) {
        return Response::json(['status' => 'fail', 'data' => '', 'message' => '请填写必要信息']);
      } 
