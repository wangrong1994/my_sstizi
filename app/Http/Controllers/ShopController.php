...
class ShopController extends Controller
{
    ...
    public function addGoods(Request $request)
    {
        if ($request->method() == 'POST') {
            ...
            $year_price = $request->get('year_price');
            $mon_price = $request->get('mon_price');
            ...
            if (empty($name) || empty($traffic) || $year_price == '' || $mon_price == '') {
            ...
            $obj->desc = $desc;
            $obj->logo = $logo;
            $obj->traffic = $traffic*1024;
            $obj->year_price = $year_price * 100; // 单位分
            $obj->mon_price = $mon_price * 100; // 单位分
            $obj->score = $score;
            $obj->type = $type;
            $obj->days = $days;
            $obj->status = $status;
    ...
    public function editGoods(Request $request)
    {
        ...
        if ($request->method() == 'POST') {
            ...
            $data = [
                ...
                'mon_price'   => $mon_price * 100, // 单位分
                'year_price'  => $year_price * 100, // 单位分
                'score'   => $score,
                'type'    => $type,
                'days'    => $days,
                'status'  => $status,
            ...
            if (!empty($goods)) {
                $goods->mon_price = $goods->mon_price / 100;
                $goods->year_price = $goods->year_price / 100;
                $goods->traffic = $goods->traffic / 1024;
            }
