...
class Kernel extends ConsoleKernel
{
    ...
    protected $commands = [
        ...
        \App\Console\Commands\AutoAddTrafficUserJob::class,
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        $schedule->command('command:autoAddTrafficUserJob')->everyFiveMinutes();
        $schedule->command('command:autoStatisticsNodeDailyTrafficJob')->dailyAt('00:00');
        $schedule->command('command:autoStatisticsNodeHourlyTrafficJob')->hourly();
        $schedule->command('command:autoStatisticsUserDailyTrafficJob')->dailyAt('00:00');
        $schedule->command('command:autoStatisticsUserHourlyTrafficJob')->hourly();
        $schedule->command('command:userExpireWarningJob')->dailyAt('03:00');
        $schedule->command('command:userTrafficWarningJob')->dailyAt('03:00');
        #$schedule->command('command:autoTransUserDataJob')->everyMinute();
